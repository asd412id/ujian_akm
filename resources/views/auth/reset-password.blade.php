	...
	<x-auth-card>
		...
		<form method="POST" action="{{ route('password.update') }}">
			...
			<div>
				...
				<x-ainput id="email" class="block mt-1 w-full" type="email" name="email" :value="old('email', $request->email)"
					required autofocus />
			</div>

			<!-- Password -->
			<div class="mt-4">
				<x-alabel for="password" :value="__('Password Baru')" />
			...
			<!-- Confirm Password -->
			<div class="mt-4">
				<x-alabel for="password_confirmation" :value="__('Konfirmasi Password Baru')" />
