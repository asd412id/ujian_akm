@php
$searchModel = $attributes->get('searchmodel');
@endphp
<div x-data="wireui_select({
    ...
    <div class="relative">
        <x-dynamic-component :component="WireUiComponent::resolve('label')" class="mb-1" :label="$label"
            :has-error="$errors->has($name) ?? false" x-on:click="togglePopover" />

        <x-dynamic-component :component="WireUiComponent::resolve('input')"
            class="cursor-pointer overflow-hidden dark:text-secondary-400" x-ref="select" x-on:click="togglePopover"
            x-on:keydown.arrow-down.prevent="$event.shiftKey || getNextFocusable().focus()"
            x-on:keydown.arrow-up.prevent="getPrevFocusable().focus()" x-bind:placeholder="getPlaceholderText()"
            x-bind:value="getValueText()" readonly :name="$name" :icon="$icon" {{ $attributes->
            ...
            <x-slot name="prepend">
                <div class="absolute left-0 inset-y-0 pl-2 pr-14 w-full flex items-center overflow-hidden cursor-pointer"
                    :class="{ 'pointer-events-none': disabled || readonly }" x-show="multiselect"
                        ...
                        <template x-for="selected in selectedOptions" :key="`selected.${selected.value}`">
                            <span class="
                                ...
                                <button class="
                                    ...
                                    <x-dynamic-component :component="WireUiComponent::resolve('icon')" class="h-3 w-3"
                                        name="x" />
                                </button>
                            </span>
                        </template>
            ...
            <x-slot name="append">
                <div class="absolute inset-y-0 right-0 flex items-center pr-2 gap-x-2">
                    ...
                    <button class="focus:outline-none" x-on:click="togglePopover" type="button">
                        <x-dynamic-component :component="WireUiComponent::resolve('icon')" class="w-5 h-5
                            {{ $errors->has($name)
    ...
    <div class="
        ...
        @if ($searchable)
        <div class="px-2 my-2">
            <x-dynamic-component :component="WireUiComponent::resolve('input')" class="
                ...
                x-on:keydown.arrow-down.prevent="$event.shiftKey || getNextFocusable().focus()"
                x-on:keydown.arrow-up.prevent="getPrevFocusable().focus()" borderless shadowless right-icon="search"
                :placeholder="trans('wireui::messages.searchHere')" wire:key="select-search"
        ...
        <ul class="max-h-60 overflow-y-auto select-none" tabindex="-1" x-ref="optionsContainer"
            ...
            <x-dynamic-component :component="data_get($option, 'component', $optionComponent)"
                :label="$getOptionLabel($key, $option)" :value="$getOptionValue($key, $option)"
                :disabled="data_get($option, 'disabled', false)" :readonly="data_get($option, 'readonly', false)"
                :option="$option" />
            @empty
            <x-dynamic-component :component="WireUiComponent::resolve('select.option')" class="text-secondary-500"
                x-on:click="closePopover" :label="trans('wireui::messages.empty_options')" readonly />
            @endforelse
            @else {{ $slot }} @endif
        </ul>
    </div>
