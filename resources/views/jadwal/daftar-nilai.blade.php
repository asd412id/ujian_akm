...
<head>
  <meta charset="utf-8">
  <title>Daftar Nilai {{ $jadwal->name }}</title>
...
<body>
  ...
  <div class="mt-2">
    <h4 class="text-center" style="font-weight:bold;padding:0;margin: 0">DAFTAR NILAI</h4>
    ...
    <table class="w-full">
      <thead>
        <tr>
          ...
          <th style="border: solid 1px #000" class="p-2 border-b-2 text-center">Nilai</th>
        </tr>
      </thead>
      <tbody>
        @php
        ...
        <tr>
          ...
          <td style="border: solid 1px #000" class="px-2 py-3 text-center">
            {{ round($p->tests()->where('jadwal_id',$jadwal->id)->select(DB::Raw('SUM(pscore) as
            nilai'))->get()[0]->nilai,2) }}
          </td>
        </tr>
        @endforeach
